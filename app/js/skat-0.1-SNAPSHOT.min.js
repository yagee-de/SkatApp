 ...
 */
define("Skat",["jquery","jqm-init","jquery.mobile","Class"],function(e,c,a,b){var d=b.extend({init:function(){},month:["Januar","Februar","März","April","Mai","Juni","Juli","August","September","Oktober","November","Dezember"],day:["Sonntag","Montag","Dienstag","Mittwoch","Donnerstag","Freitag","Sonnabend"],bids:[18,20,22,23,24,27,30,33,35,36,40,44,45,46,48,50,54,55,59,60,63,66,70,72,77,80,81,84,88,90,96,99,100,108,110,117,120,121,126,130,132,135,140,143,144,150,153,154,156,160,162,165,168,170,176,180,187,192,198,204,216,240,264],currentGame:-1,form:{createComplete:false},storage:{games:"games",players:"players",groups:"groups",dbURL:"dbURL"},api:{groups:"JSON/groups",players:"JSON/players",games:"JSON/games"},prepareGameHandler:function(g){var f=parseInt(e(g.currentTarget).attr("data-preparegame"),10);this.prepareGame(f)},onUpdateReady:function(){window.alert("SkatApp aktualisiert")},initBiddingValues:function(){var f="#bid";e(f).empty();e(f).append('<option value="0">Ramsch</option>');e.each(this.bids,function(g,j){var h=e("<option>").text(j).attr("value",j);if(g===0){h.attr("selected",true)}e(f).append(h)})},initPlayers:function(){var h=(this.load("players")||[]),g="#player",f;e(g).empty();f=e("<option>").attr("selected",true);e(g).append(f);e.each(h,function(k,j){f=e("<option>").text(j).attr("value",j);e(g).append(f)})},initGroups:function(){var f=(this.load("groups")||[]),g="#group";e(g).empty();e.each(f,function(h,k){var j=e("<option>").text(k).attr("value",k);if(h===0){j.attr("selected",true)}e(g).append(j)})},initJacks:function(){var f="#jacks",g,h;e(f).empty();for(g=1;g<12;g++){h=e("<option>").text(g).attr("value",g);if(g===0){h.attr("selected",true)}e(f).append(h)}},initGames:function(){var g=e("#gameList"),h=(this.load("games")||[]),f=null;g.empty();e.each(h,e.proxy(function(l,n){var k=this.gameDate(n),j,m;if(k!==f){f=k;g.append('<li data-role="list-divider">'+k+"</li>")}j=e("<li/>");j.addClass(n.won?"won":"lost");m=e('<a href="form.html" data-transition="slide" data-prepareGame="'+l+'"/>');j.append(m);this.gameInfo(n,m);g.append(j)},this));g.listview("refresh")},initSettings:function(){var g=this.load("dbURL"),k=e("#playerList"),j=(this.load("players")||[]),h=e("#groupList"),f=(this.load("groups")||[]);e("#dbURL").val(g);e("#playerList .player").remove();e.each(j,function(m,l){e('<li class="player"/>').text(l).appendTo(k)});k.listview("refresh");e("#groupList .group").remove();e.each(f,function(l,m){e('<li class="group"/>').text(m).appendTo(h)});h.listview("refresh")},prepareGame:function(f){this.currentGame=typeof f==="number"?f:-1;return false},fillForm:function(g){if(g<0){e("#deleteButton").parentsUntil("#inputForm","div.ui-btn").hide();e("#formPage").children(":jqmData(role=header)").children("h1").text("Spiel anlegen");return}var h=this.load("games")||[],f;if(g>=h.length){return}f=h[g];e("#formPage").children(":jqmData(role=header)").children("h1").text("Spiel Nr. "+(g+1)+" bearbeiten");e("#won").attr("checked",f.won?true:false);e("#hand").attr("checked",f.hand?true:false);this.updateGameLevel();e("#group").val(f.group);e("#player").val(f.player);e("#bid").val(f.bid).trigger("onchange");e("#jacks").val(f.jacks);e("#gameType").val(f.gameType);e("#gameLevel").val(f.gameLevel);e("#announcement").val(f.announcement);e("#timeCreated").val((new Date(f.createDate)).toISOString());e("#points").val(f.points?f.points:40);e("#deleteButton").parentsUntil("#inputForm","div.ui-btn").show()},resetForm:function(){e("#won").attr("checked",true);e("#hand").attr("checked",false);this.updateGameLevel();e("#player").val(e(this).prop("defaultSelected"));e("#bid").val(18).trigger("onchange");e("#jacks").val(1);e("#gameType").val(9);e("#gameLevel").val(1);e("#announcement").val(1);if(e("#timeCreated").val().length===0){e("#timeCreated").val((new Date()).toISOString())}e("#points").val(40)},refreshForm:function(){e("#won").checkboxradio("refresh");e("#hand").checkboxradio("refresh");e("#player").selectmenu("refresh");e("#group").selectmenu("refresh");e("#bid").selectmenu("refresh");e("#jacks").selectmenu("refresh");e("#gameType").selectmenu("refresh");e("#gameLevel").selectmenu("refresh");e("#announcement").selectmenu("refresh");e("#points").slider("refresh")},clearStorage:function(){window.localStorage.clear()},updateScore:function(){var g=this.getGame(),f=this.gamePoints(g);document.getElementById("p1").innerHTML="Punkte: "+f},bidChange:function(g){var f=parseInt(g.value,10);g.old=g.recent;g.recent=f;if((f===0&&g.old!==0)||(g.old===0&&f!==0)){e("#points").parentsUntil("#inputForm","div.ui-field-contain").toggle();e("#hand").parentsUntil("#inputForm","div.ui-field-contain").toggle();e("#gameType").parentsUntil("#inputForm","div.ui-field-contain").toggle();e("#announcement").parentsUntil("#inputForm","div.ui-field-contain").toggle();e("#jacks").parentsUntil("#inputForm","div.ui-field-contain").toggle();e("#won").parentsUntil("#inputForm","div.ui-field-contain").toggle();this.updateGameLevel()}},updateGameLevel:function(){if(parseInt(e("#bid").val(),10)===0){document.getElementById("gameLevel").innerHTML="<option value=-1>Normal</option><option value=-2>Jungfer (*-2)</option><option value=2>Durchmarsch (*2)</option>"}else{if(e("#hand").attr("checked")){document.getElementById("gameLevel").innerHTML="<option value=2>Hand (+2)</option><option value=3>Schneider (+3)</option><option value=4>Schneider angesagt (+4)</option><option value=5>Schwarz (+5)</option><option value=6>Schwarz angesagt (+6)</option><option value=7>Ouvert (+7)</option>"}else{document.getElementById("gameLevel").innerHTML="<option value=1>Normal (+1)</option><option value=2>Schneider (+2)</option><option value=3>Schwarz (+3)</option>"}}if(this.form.createComplete){e("#gameLevel").selectmenu("refresh")}},getGame:function(){var m=e("#group").val(),n=e("#player").val(),h=parseInt(e("#bid").val(),10),g=parseInt(e("#gameLevel").val(),10),o=new Date().getTime(),f=o,k,p,l;try{k=e("#timeCreated").val();if(k.length>0){k=new Date(k);f=k.getTime()}}catch(j){window.alert(j)}p={group:m,player:n,bid:h,gameLevel:g,createDate:f,modifyDate:o};if(h===0){l=parseInt(e("#points").val(),10);p.points=(g===2)?120:l;p.won=(g===2)}else{p.jacks=parseInt(e("#jacks").val(),10);p.gameType=parseInt(e("#gameType").val(),10);p.hand=e("#hand").attr("checked")?true:false;p.announcement=parseInt(e("#announcement").val(),10);p.won=e("#won").attr("checked")?true:false}return p},getExport:function(){var f=this.load("games")||[];e.each(f,e.proxy(function(h,g){var j=this.gamePoints(g);g.value=j},this));return f},load:function(f,g){return JSON.parse(window.localStorage.getItem(this.storage[f]))},store:function(f,g){window.localStorage.setItem(this.storage[f],JSON.stringify(g))},storeGame:function(){var f=this.getGame(),g;if(f.player.length===0){window.alert("Kein Spieler ausgewählt");e.mobile.silentScroll(0);return}g=this.load("games")||[];if(this.currentGame===-1){g.push(f);this.resetForm();this.refreshForm()}else{g[this.currentGame]=f}this.store("games",g);window.alert("Spiel gespeichert");if(this.currentGame===-1){e.mobile.silentScroll(0)}else{location.back()}},deleteGame:function(f){var g=this.load("games")||[];if(f<0||f>=g.length){return false}g.splice(f,1);this.store("games",g);return true},removeGames:function(){this.store("games",[]);return true},loadGame:function(g){var h=(this.load("games")||[]),f;if(g<0||g>=h.length){return false}f=h[g];this.updateForm(f);this.currentGame=g;return true},updateForm:function(f){e("#group").val(f.group).selectmenu("refresh");e("#player").val(f.player).selectmenu("refresh");e("#bid").val(f.bid).selectmenu("refresh");e("#jacks").val(f.jacks).selectmenu("refresh");e("#gameType").val(f.gameType).selectmenu("refresh");e("#hand").attr("checked",f.hand).checkboxradio("refresh");e("#gameLevel").val(f.gameLevel).selectmenu("refresh");e("#announcement").val(f.announcement).selectmenu("refresh");e("#won").attr("checked",f.won).checkboxradio("refresh")},gameTypeName:function(f){switch(f){case 9:return"Schellen";case 10:return"Herz";case 11:return"Grün";case 12:return"Eichel";case 23:return"Null";case 24:return"Grand";case 35:return"Null Hand";case 46:return"Null Ouvert";case 59:return"Null Ouvert Hand";default:return"unbekannt"}},gamePoints:function(h){var g,f=h.gameType;if(h.bid===0){if(h.gameLevel===2){return 240}return h.points*h.gameLevel}else{if(f===23||f===35||f===46||f===59){g=f}else{g=f*(h.jacks+h.gameLevel)}}g*=h.announcement;if(!h.won){g*=-2}return g},gameClock:function(f){var g=new Date(f.createDate);return(g.getHours()<10?"0":"")+g.getHours()+":"+(g.getMinutes()<10?"0":"")+g.getMinutes()},gameDate:function(f){var g=new Date(f.createDate);return this.day[g.getDay()]+", "+g.getDate()+". "+this.month[g.getMonth()]+" "+g.getFullYear()},gameInfo:function(f,g){g.append("<h3>"+f.player+"</h3>");g.append("<p><strong>"+(f.won?"Gewonnen":"Verloren")+"</strong></p>");g.append("<p>"+(f.bid?this.gameTypeName(f.gameType):"Ramsch")+" Punkte:"+this.gamePoints(f)+"</p>");g.append('<p class="ui-li-aside"><strong>'+this.gameClock(f)+"</strong></p>")},addPlayer:function(h){var g=(this.load("players")||[]),f;for(f=0;f<g.lenght;f++){if(g[f]===h){return}}g.push(h);g.sort();this.store("players",g)},addGroup:function(h){var f=(this.load("groups")||[]),g;for(g=0;g<f.lenght;g++){if(f[g]===h){return}}f.push(h);this.store("groups",f)},setDbURL:function(f){this.store("dbURL",f)},downloadPlayer:function(){this.apiCall(this.api.players,function(f){f.sort();this.store("players",f);window.alert(f.length+" Spieler geladen.")})},downloadGroups:function(){this.apiCall(this.api.groups,function(f){f.sort();this.store("groups",f);window.alert(f.length+" Gruppen geladen.")})},apiCall:function(h,k){var j=this.load("dbURL"),f,g=this;if(j===null||j.length===0){window.alert("Keine SKatDB-URL definiert");return}f=j+h;e.ajax({url:f,type:"GET",crossDomain:true,dataType:"json",error:function(m,l){window.alert("Fehler: "+m.status+" ("+m.statusText+")");if(window.confirm('URL "'+f+'" laden?')){window.location.href=f}},success:function(l){k.apply(g,[l])}})},uploadGames:function(){var g=this.load("dbURL"),h,f=this;if(g===null||g.length===0){window.alert("Keine SKatDB-URL definiert");return}h=this.getExport();if(h.length===0){window.alert("Keine lokalen Spiele zum Übertragen vorhanden.");return}e.ajax({url:g+this.api.games,type:"POST",crossDomain:true,data:{games:JSON.stringify(h)},dataType:"json",error:function(k,j){window.alert("Fehler: "+k.status+" ("+k.statusText+"):\n"+k.responseText)},success:function(l){var j=h.length,k;if(l.handList){k=l.handList;f.store("games",k);j-=k.length}else{f.removeGames()}window.alert(j+" von "+h.length+" Spielen hochgeladen:\n"+l.msg)}})}});return d});define("skat-init",["jquery","jqm-init","jquery.mobile","Skat"],function(e,b,a,c){var d=new c();window.skat=d;window.addEventListener("load",function(f){if(window.applicationCache){window.applicationCache.addEventListener("updateready",function(g){if(window.applicationCache.status===window.applicationCache.UPDATEREADY){window.applicationCache.swapCache();if(window.confirm("Neue Version von SkatApp verf�gbar. Neu Laden?")){window.location.reload()}}},false)}},false);e(document).ready(function(){e("*[data-preparegame]").live("vclick",function(f){d.prepareGameHandler(f)})});e(document).bind("pageinit",function(){});e(document).delegate("#formPage","pagebeforecreate",function(){d.form.createComplete=false;d.initBiddingValues();d.initJacks();d.initPlayers();d.initGroups();d.resetForm();d.form.createComplete=true});e(document).delegate("#formPage","pagebeforeshow",function(f,g){d.initPlayers();d.initGroups();d.fillForm(d.currentGame);d.refreshForm();d.updateScore()});e(document).delegate("#gamesPage","pagebeforeshow",function(){d.initGames()});e(document).delegate("#settingsPage","pagebeforeshow",function(){d.initSettings()});e(document).change(function(){d.updateScore()})});