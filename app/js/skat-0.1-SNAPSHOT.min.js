 ...
 */
define("Class",["jquery"],function(c){var b=false,a;a=function(){};a.extend=function d(k){var h=this.prototype,j=this,g,f;b=true;g=new j();b=false;c.each(k,function(l,m){g[l]=typeof m==="function"&&typeof h[l]==="function"?function(){var o=this._super,n;this._super=h[l];n=m.apply(this,arguments);this._super=o;return n}:m});function e(){if(!b&&this.init){this.init.apply(this,arguments)}}e.prototype=g;e.prototype.constructor=e;e.extend=d;return e};return a});define("SkatGame",["Class"],function(c){var e,b,d,a;e=["Januar","Februar","M�rz","April","Mai","Juni","Juli","August","September","Oktober","November","Dezember"];b=["Sonntag","Montag","Dienstag","Mittwoch","Donnerstag","Freitag","Sonnabend"];d=[18,20,22,23,24,27,30,33,35,36,40,44,45,46,48,50,54,55,59,60,63,66,70,72,77,80,81,84,88,90,96,99,100,108,110,117,120,121,126,130,132,135,140,143,144,150,153,154,156,160,162,165,168,170,176,180,187,192,198,204,216,240,264];a=c.extend({init:function(g){var h=this,f;if(typeof g==="string"){JSON.parse(g,function(j,k){h.reviver(j,k)})}else{if(typeof g==="object"){for(f in g){this.reviver(f,g[f])}}else{this.created=new Date();this.modified=new Date()}}},group:null,player:null,bid:0,gameLevel:0,createDate:0,modifydDate:0,points:0,jacks:0,gameType:0,hand:false,won:true,announcement:1,toJSON:function(){var f={group:this.group,player:this.player,bid:this.bid,gameLevel:this.gameLevel,createDate:this.createDate.getTime(),modifyDate:this.modifyDate.getTime()};if(this.isRamsch()){f.points=(this.gameLevel===2)?120:this.points;f.won=(this.gameLevel===2)}else{f.jacks=this.jacks;f.gameType=this.gameType;f.hand=this.hand;f.announcement=this.announcement;f.won=this.won}return f},reviver:function(g,j){switch(g){case"group":case"player":if(typeof j==="string"){this[g]=j}else{throw g+" is not a string"}break;case"bid":if(typeof j==="number"){var h=(j===0),f;if(!h){for(f=0;f<d.length;f++){if(d[f]===j){h=true;break}}}if(!h){throw"bid has not a valid value"}this.bid=j}else{throw g+" is not a number"}break;case"gameLevel":case"jacks":case"gameType":case"announcement":case"points":if(typeof j==="number"){this[g]=j}else{throw g+" is not a number"}break;case"won":case"hand":if(typeof j==="boolean"){this[g]=j}else{throw g+" is not a boolean value"}break;case"createDate":case"modifyDate":if(typeof j==="number"&&j>new Date(2000,0,1,0,0,0,0).getTime()){this[g]=new Date(j)}else{throw g+" is not a valid date value"}break;default:window.alert("Ignored unknown key: "+g+", value: "+j);break}},isRamsch:function(){return this.bid===0},getGameTypeName:function(){switch(this.gameType){case 9:return"Schellen";case 10:return"Herz";case 11:return"Gr�n";case 12:return"Eichel";case 23:return"Null";case 24:return"Grand";case 35:return"Null Hand";case 46:return"Null Ouvert";case 59:return"Null Ouvert Hand";default:return"unbekannt"}},getTimeString:function(){var f=new Date(this.createDate);return(f.getHours()<10?"0":"")+f.getHours()+":"+(f.getMinutes()<10?"0":"")+f.getMinutes()},getDateString:function(){var f=new Date(this.createDate);return b[f.getDay()]+", "+f.getDate()+". "+e[f.getMonth()]+" "+f.getFullYear()},getPoints:function(){var g,f=this.gameType;if(this.isRamsch()){if(this.gameLevel===2){return 240}return this.points*this.gameLevel}else{if(f===23||f===35||f===46||f===59){g=f}else{g=f*(this.jacks+this.gameLevel)}}g*=this.announcement;if(!this.won){g*=-2}return g}});a.bids=d;a.month=e;a.day=b;return a});define("SkatStorage",["jquery","jqm-init","jquery.mobile","Class","SkatGame"],function(f,d,a,c,b){var e=c.extend({init:function(){},storage:{games:"games",players:"players",groups:"groups",dbURL:"dbURL"},load:function(g){return JSON.parse(window.localStorage.getItem(this.storage[g]))},store:function(g,h){window.localStorage.setItem(this.storage[g],JSON.stringify(h))},deleteGame:function(g){var h=this.load("games")||[];if(g<0||g>=h.length){return false}if(window.confirm("Spiel Nr."+(g+1)+" wirklich l�schen?")){h.splice(g,1);this.store("games",h);return true}return false},removeGames:function(){this._skat.store("games",[]);return true},clearStorage:function(){window.localStorage.clear()}});return e});define("SkatGames",["jquery","jqm-init","jquery.mobile","SkatGame","SkatStorage"],function(f,d,a,b,e){var c=e.extend({init:function(){},initGames:function(){var h=f("#gameList"),j=(this.load("games")||[]),g=null;h.empty();f.each(j,f.proxy(function(n,p){var m=new b(p),l=m.getDateString(),k,o;if(l!==g){g=l;h.append('<li data-role="list-divider">'+l+"</li>")}k=f("<li/>");k.addClass(m.won?"won":"lost");o=f('<a href="form.html" data-transition="slide" data-prepareGame="'+n+'"/>');k.append(o);this.appendInfo(m,o);h.append(k)},this));h.listview("refresh")},appendInfo:function(g,h){h.append("<h3>"+g.player+"</h3>");h.append("<p><strong>"+(g.won?"Gewonnen":"Verloren")+"</strong></p>");h.append("<p>"+(g.bid?g.getGameTypeName():"Ramsch")+" Punkte:"+g.getPoints()+"</p>");h.append('<p class="ui-li-aside"><strong>'+g.getTimeString()+"</strong></p>")}});return c});define("SkatForm",["jquery","jqm-init","jquery.mobile","SkatGame","SkatStorage"],function(f,c,a,b,e){var d=e.extend({init:function(){},currentGame:-1,createComplete:false,initBiddingValues:function(){var g="#bid";f(g).empty();f(g).append('<option value="0">Ramsch</option>');f.each(b.bids,function(h,k){var j=f("<option>").text(k).attr("value",k);if(h===0){j.attr("selected",true)}f(g).append(j)})},initPlayers:function(){var j=(this.load("players")||[]),h="#player",g;f(h).empty();g=f("<option>").attr("selected",true);f(h).append(g);f.each(j,function(l,k){g=f("<option>").text(k).attr("value",k);f(h).append(g)})},initGroups:function(){var g=(this.load("groups")||[]),h="#group";f(h).empty();f.each(g,function(j,l){var k=f("<option>").text(l).attr("value",l);if(j===0){k.attr("selected",true)}f(h).append(k)})},initJacks:function(){var g="#jacks",h,j;f(g).empty();for(h=1;h<12;h++){j=f("<option>").text(h).attr("value",h);if(h===0){j.attr("selected",true)}f(g).append(j)}},prepareGame:function(g){this.currentGame=typeof g==="number"?g:-1},fillForm:function(h){if(typeof h!=="number"||h<0){f("#deleteButton").parentsUntil("#inputForm","div.ui-btn").hide();f("#formPage").children(":jqmData(role=header)").children("h1").text("Spiel anlegen");return}var j=this.load("games")||[],g;if(h>=j.length){return}g=new b(j[h]);f("#formPage").children(":jqmData(role=header)").children("h1").text("Spiel Nr. "+(h+1)+" bearbeiten");f("#won").attr("checked",g.won?true:false);f("#hand").attr("checked",g.hand?true:false);this.updateGameLevel();f("#group").val(g.group);f("#player").val(g.player);f("#bid").val(g.bid).trigger("onchange");f("#jacks").val(g.jacks);f("#gameType").val(g.gameType);f("#gameLevel").val(g.gameLevel);f("#announcement").val(g.announcement);f("#timeCreated").val(g.createDate.toISOString());f("#points").val(g.points?g.points:40);f("#deleteButton").parentsUntil("#inputForm","div.ui-btn").show()},resetForm:function(){f("#won").attr("checked",true);f("#hand").attr("checked",false);this.updateGameLevel();f("#player").val(f(this).prop("defaultSelected"));f("#bid").val(18).trigger("onchange");f("#jacks").val(1);f("#gameType").val(9);f("#gameLevel").val(1);f("#announcement").val(1);if(f("#timeCreated").val().length===0){f("#timeCreated").val((new Date()).toISOString())}f("#points").val(40)},refreshForm:function(){f("#won").checkboxradio("refresh");f("#hand").checkboxradio("refresh");f("#player").selectmenu("refresh");f("#group").selectmenu("refresh");f("#bid").selectmenu("refresh");f("#jacks").selectmenu("refresh");f("#gameType").selectmenu("refresh");f("#gameLevel").selectmenu("refresh");f("#announcement").selectmenu("refresh");f("#points").slider("refresh")},updateScore:function(){var h=this.getGame(),g=h.getPoints();document.getElementById("p1").innerHTML="Punkte: "+g},bidChange:function(h){var g=parseInt(h.value,10);switch(g){case 0:f("#points").parentsUntil("#inputForm","div.ui-field-contain").show();f("#hand").parentsUntil("#inputForm","div.ui-field-contain").hide();f("#gameType").parentsUntil("#inputForm","div.ui-field-contain").hide();f("#announcement").parentsUntil("#inputForm","div.ui-field-contain").hide();f("#jacks").parentsUntil("#inputForm","div.ui-field-contain").hide();f("#won").parentsUntil("#inputForm","div.ui-field-contain").hide();break;default:f("#points").parentsUntil("#inputForm","div.ui-field-contain").hide();f("#hand").parentsUntil("#inputForm","div.ui-field-contain").show();f("#gameType").parentsUntil("#inputForm","div.ui-field-contain").show();f("#announcement").parentsUntil("#inputForm","div.ui-field-contain").show();f("#jacks").parentsUntil("#inputForm","div.ui-field-contain").show();f("#won").parentsUntil("#inputForm","div.ui-field-contain").show();break}this.updateGameLevel()},updateGameLevel:function(){if(parseInt(f("#bid").val(),10)===0){document.getElementById("gameLevel").innerHTML="<option value=-1>Normal</option><option value=-2>Jungfer (*-2)</option><option value=2>Durchmarsch (*2)</option>"}else{if(f("#hand").attr("checked")){document.getElementById("gameLevel").innerHTML="<option value=2>Hand (+2)</option><option value=3>Schneider (+3)</option><option value=4>Schneider angesagt (+4)</option><option value=5>Schwarz (+5)</option><option value=6>Schwarz angesagt (+6)</option><option value=7>Ouvert (+7)</option>"}else{document.getElementById("gameLevel").innerHTML="<option value=1>Normal (+1)</option><option value=2>Schneider (+2)</option><option value=3>Schwarz (+3)</option>"}}if(this.createComplete){f("#gameLevel").selectmenu("refresh")}},getGame:function(){var n=f("#group").val(),o=f("#player").val(),j=parseInt(f("#bid").val(),10),h=parseInt(f("#gameLevel").val(),10),p=new Date().getTime(),g=p,l,q,m;try{l=f("#timeCreated").val();if(l.length>0){l=new Date(l);g=l.getTime()}}catch(k){window.alert(k)}q={group:n,player:o,bid:j,gameLevel:h,createDate:g,modifyDate:p};if(j===0){m=parseInt(f("#points").val(),10);q.points=(h===2)?120:m;q.won=(h===2)}else{q.jacks=parseInt(f("#jacks").val(),10);q.gameType=parseInt(f("#gameType").val(),10);q.hand=f("#hand").attr("checked")?true:false;q.announcement=parseInt(f("#announcement").val(),10);q.won=f("#won").attr("checked")?true:false}return new b(q)},storeGame:function(){var g=this.getGame().toJSON(),h;if(g.player.length===0){window.alert("Kein Spieler ausgew�hlt");f.mobile.silentScroll(0);return}h=this.load("games")||[];if(this.currentGame===-1){h.push(g)}else{h[this.currentGame]=g}this.store("games",h);window.alert("Spiel gespeichert");this.resetForm();this.refreshForm();if(this.currentGame===-1){f.mobile.silentScroll(0)}else{window.history.back()}}});return d});define("SkatSettings",["jquery","jqm-init","jquery.mobile","SkatStorage"],function(e,c,b,d){var a=d.extend({init:function(){},initSettings:function(){var g=this.load("dbURL"),k=e("#playerList"),j=(this.load("players")||[]),h=e("#groupList"),f=(this.load("groups")||[]);e("#dbURL").val(g);e("#playerList .player").remove();e.each(j,function(m,l){e('<li class="player"/>').text(l).appendTo(k)});k.listview("refresh");e("#groupList .group").remove();e.each(f,function(l,m){e('<li class="group"/>').text(m).appendTo(h)});h.listview("refresh")},addPlayer:function(h){var g=(this.load("players")||[]),f;for(f=0;f<g.lenght;f++){if(g[f]===h){return}}g.push(h);g.sort();this.store("players",g)},addGroup:function(h){var f=(this.load("groups")||[]),g;for(g=0;g<f.lenght;g++){if(f[g]===h){return}}f.push(h);this.store("groups",f)},setDbURL:function(f){this.store("dbURL",f)}});return a});define("SkatSync",["jquery","jqm-init","jquery.mobile","SkatStorage","SkatGame"],function(g,c,a,f,b){var e,d;e={groups:"JSON/groups",players:"JSON/players",games:"JSON/games"};d=f.extend({init:function(){},getExport:function(){var h=this.load("games")||[];g.each(h,function(k,j){var l=new b(j).getPoints();j.value=l});return h},downloadPlayer:function(){this.apiCall(e.players,function(h){h.sort();this.store("players",h);window.alert(h.length+" Spieler geladen.")})},downloadGroups:function(){this.apiCall(e.groups,function(h){h.sort();this.store("groups",h);window.alert(h.length+" Gruppen geladen.")})},apiCall:function(k,m){var l=this.load("dbURL"),h,j=this;if(l===null||l.length===0){window.alert("Keine SKatDB-URL definiert");return}h=l+k;g.ajax({url:h,type:"GET",crossDomain:true,dataType:"json",error:function(o,n){window.alert("Fehler: "+o.status+" ("+o.statusText+")");if(window.confirm('URL "'+h+'" laden?')){window.location.href=h}},success:function(n){m.apply(j,[n])}})},uploadGames:function(){var j=this.load("dbURL"),k,h=this;if(j===null||j.length===0){window.alert("Keine SKatDB-URL definiert");return}k=this.getExport();if(k.length===0){window.alert("Keine lokalen Spiele zum �bertragen vorhanden.");return}g.ajax({url:j+e.games,type:"POST",crossDomain:true,data:{games:JSON.stringify(k)},dataType:"json",error:function(m,l){window.alert("Fehler: "+m.status+" ("+m.statusText+"):\n"+m.responseText)},success:function(n){var l=k.length,m;if(n.handList){m=n.handList;h._skat.store("games",m);l-=m.length}else{h._skat.settings.removeGames()}window.alert(l+" von "+k.length+" Spielen hochgeladen:\n"+n.msg)}})}});d.api=e;return d});define("Skat",["jquery","jqm-init","jquery.mobile","Class","SkatForm","SkatGame","SkatGames","SkatSettings","SkatSync"],function(j,a,k,b,c,h,g,d,e){var f=b.extend({init:function(){this.form=new c();this.games=new g();this.settings=new d();this.sync=new e()},form:null,games:null,settings:null,sync:null,prepareGameHandler:function(m){var l=parseInt(j(m.currentTarget).attr("data-preparegame"),10);this.form.prepareGame(l)}});return f});define("skat-init",["jquery","jqm-init","jquery.mobile","Skat"],function(e,b,a,c){var d=new c();window.skat=d;window.addEventListener("load",function(f){if(window.applicationCache){window.applicationCache.addEventListener("updateready",function(g){if(window.applicationCache.status===window.applicationCache.UPDATEREADY){window.applicationCache.swapCache();if(window.confirm("Neue Version von SkatApp verf�gbar. Neu Laden?")){window.location.reload()}}},false)}},false);e(document).ready(function(){e("*[data-preparegame]").live("vclick",function(f){d.prepareGameHandler(f)})});e(document).bind("pageinit",function(){});e(document).delegate("#formPage","pagebeforecreate",function(){d.form.createComplete=false;d.form.initBiddingValues();d.form.initJacks();d.form.initPlayers();d.form.initGroups();d.form.resetForm();d.form.createComplete=true});e(document).delegate("#formPage","pagebeforeshow",function(f,g){d.form.initPlayers();d.form.initGroups();d.form.fillForm(d.form.currentGame);d.form.refreshForm();d.form.updateScore()});e(document).delegate("#gamesPage","pagebeforeshow",function(){d.games.initGames()});e(document).delegate("#settingsPage","pagebeforeshow",function(){d.settings.initSettings()});e(document).change(function(){d.form.updateScore()})});